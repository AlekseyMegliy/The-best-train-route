      ...
      reader.onload = function(e) {
        ...
        while(vertex.length != station_visited.length){
          ...
          for(let i=0; i<XL_row_object.length; i++){
            ...
            let time_s_min = +XL_row_object[i]["Time_s"].split(":")[0] * 60 +  +XL_row_object[i]["Time_s"].split(":")[1]; 
            let time_f_min = +XL_row_object[i]["Time_f"].split(":")[0] * 60 +  +XL_row_object[i]["Time_f"].split(":")[1]; 
            ...
            if( now_start == XL_row_object[i]["Start"] 
              ...
              // Нужно узнать сколько времени пройдёт между приездом предыдущего поезда и отъездом рассматриваемого
               let wait_time;
              if(its_time_to_stop_okey  > time_s_min){
                wait_time =   its_time_to_stop_okey - time_s_min;}
               else{
                wait_time = 24*60 + its_time_to_stop_okey - time_s_min;
              }
              ...
              // прошедших станций + время ожидания + время проезда к этой станции. Либо её "Стоимости" до этого шага, в зависимости от того, что меньше 
              if(vertexes[vertex.indexOf(XL_row_object[i]["Fin"])]["Cost"] > cost_now + + wait_time +XL_row_object[i]["Full_time"]){
                vertexes[vertex.indexOf(XL_row_object[i]["Fin"])]["Cost"] = cost_now + + wait_time +XL_row_object[i]["Full_time"]
            ...
            // Добавляем к текущей стоимости пути время поездки к следующей станции И время ожидания, которое найдём уже знакомым способом 
            // let split_time_start_cheap = cheapest["Time_s"].split(":");
            let time_s_min = +cheapest["Time_s"].split(":")[0] * 60 +  +cheapest["Time_s"].split(":")[1]; 
            
            if(its_time_to_stop_okey  > time_s_min){
                let = wait_time_cheap =   its_time_to_stop_okey - time_s_min;}
             else{
                let = wait_time_cheap = 24*60 + its_time_to_stop_okey - time_s_min;
            }

            cost_now += wait_time_cheap + +cheapest["Full_time"];

            // Делаем конечную станцию этого поезда начальной для следующего 
            now_start = cheapest["Fin"];
              
            // Запишем время прибытия поезда на следующую станцию 
             let cheap_time_fin = cheapest["Time_f"].split(":");
             its_time_to_stop_okey = +cheap_time_fin[0] * 60 +  +cheap_time_fin[1]; 
          ...
          "All the time for the trip: " + Math.floor(Math.floor(cost_now/(60*24))) + " Days " +  Math.floor((cost_now%(60*24))/60) + " Hours " + cost_now%60 + " Minutes "  
          );
