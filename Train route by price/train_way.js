      ...
      while(vertex.length != station_visited.length){
        ...
        for(let i=0; i<XL_row_object.length; i++){
          ...
          if( now_start == XL_row_object[i]["Start"] 
            ...
            // Находим в катоалоге станций индекс конечной станции рассматриваемого поезда чтобы посмотреть его цену
            // если станции маршрута данного поезда ещё не было посещены, то "Стоимость" конечной становится равной "Стоимости" 
            // прошедших станций + цена проезда к этой станции. Либо её "Стоимости" до этого шага, в зависимости от того, что меньше 
            if(vertexes[vertex.indexOf(XL_row_object[i]["Fin"])]["Cost"] > cost_now + +XL_row_object[i]["Pay"]){
              vertexes[vertex.indexOf(XL_row_object[i]["Fin"])]["Cost"] = cost_now + +XL_row_object[i]["Pay"]
          ...
          // Добавляем к текущей стоимости пути стоимость поездки к следующей станции
          cost_now += +cheapest["Pay"];
          ...
          sometimes_cost = [];
          // return cheapest["Fin"];
      // testing_one(XL_row_object[0]["Start"]);
